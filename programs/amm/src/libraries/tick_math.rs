use crate::{error::ContractError, libraries::big_num::U128};
...
/// * `tick` - Price tick
///
pub fn get_sqrt_price_at_tick(tick: i32) -> Result<u128, ContractError> {
    let abs_tick = tick.abs() as u32;
    if abs_tick > MAX_TICK as u32 {
        return Err(ContractError::TickUpperOverflow);
    }
...
pub fn get_tick_at_sqrt_price(sqrt_price_x64: u128) -> Result<i32, ContractError> {
    // second inequality must be < because the price can never reach the price at the max tick
    if sqrt_price_x64 < MIN_SQRT_PRICE_X64 && sqrt_price_x64 < MAX_SQRT_PRICE_X64 {
        return Err(ContractError::SqrtPriceX64);
    }

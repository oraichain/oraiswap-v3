...
use cosmwasm_std::{
    from_json, to_json_binary, Addr, BankMsg, Binary, Coin, CosmosMsg, Deps, DepsMut, Env, Int256,
    MessageInfo, Response, StdResult, Storage, Uint128, Uint256, WasmMsg,
};
use cw20::Cw20ExecuteMsg;
use cw721::{AllNftInfoResponse, Cw721ExecuteMsg, Cw721QueryMsg, Cw721ReceiveMsg, NftInfoResponse};
use ruint::Uint;
use utils::{
    add_delta, compute_swap_step, get_amount_0_delta, get_amount_1_delta, get_fee_growth_inside,
    get_sqrt_ratio_at_tick, get_tick_at_sqrt_ratio, get_tokens_owed, mul_div, FeeGrowthOutside,
    MAX_SQRT_RATIO, MAX_TICK, MIN_SQRT_RATIO, MIN_TICK, Q128,
};
...
pub fn instantiate(
    ...
    FEE_GROWTH_GROBAL.save(
        deps.storage,
        &FeeGrowthGlobal {
            fee_growth_global_0_x128: Uint256::zero(),
            fee_growth_global_1_x128: Uint256::zero(),
        },
    )?;
    Ok(Response::default())
}
...
fn execute_swap(
    ...
    if zero_for_one {
        if !(sqrt_price_limit_x96 < current_state.sqrt_price_x96
            && sqrt_price_limit_x96 > Uint256::from_be_bytes(MIN_SQRT_RATIO.to_be_bytes()))
        {
            return Err(ContractError::InvalidPriceLimit {});
        }
    } else if !(sqrt_price_limit_x96 > current_state.sqrt_price_x96
        && sqrt_price_limit_x96 < Uint256::from_be_bytes(MAX_SQRT_RATIO.to_be_bytes()))
    {
        return Err(ContractError::InvalidPriceLimit {});
    }
    ...
    let fee_growth_global_x128 = if zero_for_one {
        fee_growth_global.fee_growth_global_0_x128
    } else {
        fee_growth_global.fee_growth_global_1_x128
    };
        ...
        && state.sqrt_price_x96 != sqrt_price_limit_x96
    {
        let mut step = StepComputations {
            sqrt_price_start_x96: Uint256::zero(),
            tick_next: 0,
            initialized: false,
            sqrt_price_next_x96: Uint256::zero(),
        ...
        step.sqrt_price_next_x96 = Uint256::from_be_bytes(
            get_sqrt_ratio_at_tick(step.tick_next)
                .unwrap()
                .to_be_bytes(),
        );
        ...
        let new_sqrt_ratio_target_x96;
        if precheck {
            new_sqrt_ratio_target_x96 = sqrt_price_limit_x96;
        } else {
            new_sqrt_ratio_target_x96 = step.sqrt_price_next_x96;
        }

        if let Ok((sqrt_price_next_x96, amount_in, amount_out, fee_amount)) = compute_swap_step(
            Uint::<256, 4>::from_be_bytes(state.sqrt_price_x96.to_be_bytes()),
            Uint::<256, 4>::from_be_bytes(new_sqrt_ratio_target_x96.to_be_bytes()),
            state.liquidity,
            I256::from_be_bytes(state.amount_specified_remaining.to_be_bytes()),
            config.fee.into(),
        ) {
            state.sqrt_price_x96 = Uint256::from_be_bytes(sqrt_price_next_x96.to_be_bytes());
            step.amount_in = Uint256::from_be_bytes(amount_in.to_be_bytes());
            step.amount_out = Uint256::from_be_bytes(amount_out.to_be_bytes());
            step.fee_amount = Uint256::from_be_bytes(fee_amount.to_be_bytes());
        } else {
            return Err(ContractError::CannotComputeSwapStep {});
        }

        if exact_input {
            state.amount_specified_remaining -=
                Int256::from_be_bytes((step.amount_in + step.fee_amount).to_be_bytes());
            state.amount_calculated -= Int256::from_be_bytes((step.amount_out).to_be_bytes())
        } else {
            state.amount_specified_remaining +=
                Int256::from_be_bytes(step.amount_out.to_be_bytes());
            state.amount_calculated +=
                Int256::from_be_bytes((step.amount_in + step.fee_amount).to_be_bytes())
        }

        if state.liquidity > 0u128 {
            state.fee_growth_global_x128 += Uint256::from_be_bytes(
                mul_div(
                    Uint::<256, 4>::from_be_bytes(step.fee_amount.to_be_bytes()),
                    Q128,
                    Uint::<256, 4>::from_be_bytes(state.liquidity.to_be_bytes()),
                )
                .unwrap()
                .to_be_bytes(),
            );
        ...
        if state.sqrt_price_x96 == step.sqrt_price_next_x96 {
            if step.initialized {
                ...
                let var3;
                if zero_for_one {
                    var2 = state.fee_growth_global_x128;
                    var3 = fee_growth_global.fee_growth_global_0_x128;
                } else {
                    var2 = fee_growth_global.fee_growth_global_1_x128;
                    var3 = state.fee_growth_global_x128
                }
            ...
            state.tick = get_tick_at_sqrt_ratio(Uint::<256, 4>::from_be_bytes(
                state.sqrt_price_x96.to_be_bytes(),
            ))
            .unwrap();
        }
    ...
    if state.tick != current_state.tick {
        current_state.sqrt_price_x96 = state.sqrt_price_x96;
    ...
    if zero_for_one {
        fee_growth_global.fee_growth_global_0_x128 = state.fee_growth_global_x128;
    } else {
        fee_growth_global.fee_growth_global_1_x128 = state.fee_growth_global_x128
    }
    ...
    if zero_for_one {
        if amount1 < Int256::zero() {
            let amount1_format = Uint128::try_from(amount1.abs()).unwrap();
            let amount0_format = Uint128::try_from(amount0).unwrap();
        ...
        if amount0 < Int256::zero() {
            let amount0_format = Uint128::try_from(amount0.abs()).unwrap();
            let amount1_format = Uint128::try_from(amount1).unwrap();
            match config.token_1 {
                AssetInfo::NativeToken { denom } => {
                    if !(info.funds.contains(&Coin {
                        denom: denom.clone(),
...
fn cross(
    ...
    cur_tick_info.fee_growth_outside_0_x128 =
        fee_growth_global_0x128 - cur_tick_info.fee_growth_outside_0_x128;
    cur_tick_info.fee_growth_outside_1_x128 =
        fee_growth_global_1x128 - cur_tick_info.fee_growth_outside_1_x128;
    cur_tick_info
}
...
fn update_tick_and_list_initialized_tick(
        ...
        None => {
            ...
            if tick <= current_tick {
                TickInfo {
                    liquidity_gross: 0,
                    liquidity_net: 0,
                    fee_growth_outside_0_x128: fee_growth_global.fee_growth_global_0_x128,
                    fee_growth_outside_1_x128: fee_growth_global.fee_growth_global_1_x128,
                }
            } else {
                TickInfo {
                    liquidity_gross: 0,
    ...
    if liquidity_delta != 0 {
        new_tick_info.liquidity_gross =
            add_delta(new_tick_info.liquidity_gross, liquidity_delta).unwrap();
...
fn modiy_position(
    ...
    let mut current_state = match CURRENT_STATE.may_load(storage)? {
        ...
        None => {
            let tick_lower_sqrt_price_x96 = get_sqrt_ratio_at_tick(tick_lower).unwrap();
            let tick_upper_sqrt_price_x96 = get_sqrt_ratio_at_tick(tick_upper).unwrap();
            let sqrt_price_x96 =
                (tick_lower_sqrt_price_x96 + tick_upper_sqrt_price_x96) / Uint::<256, 4>::from(2);
            CurrentState {
                liquidity: 0,
                sqrt_price_x96: Uint256::from_be_bytes(sqrt_price_x96.to_be_bytes()),
                tick: get_tick_at_sqrt_ratio(sqrt_price_x96).unwrap(),
            }
        }
    };
    ...
    let (fee_growth_inside_0_x128, fee_growth_inside_1_x128) = get_fee_growth_inside(
        FeeGrowthOutside {
            fee_growth_outside0_x128: Uint::<256, 4>::from_be_bytes(
                tick_lower_info.fee_growth_outside_0_x128.to_be_bytes(),
            ),
            fee_growth_outside1_x128: Uint::<256, 4>::from_be_bytes(
                tick_lower_info.fee_growth_outside_1_x128.to_be_bytes(),
            ),
        },
        FeeGrowthOutside {
            fee_growth_outside0_x128: Uint::<256, 4>::from_be_bytes(
                tick_upper_info.fee_growth_outside_0_x128.to_be_bytes(),
            ),
            fee_growth_outside1_x128: Uint::<256, 4>::from_be_bytes(
                tick_upper_info.fee_growth_outside_1_x128.to_be_bytes(),
            ),
        },
        tick_lower,
        tick_upper,
        current_state.tick,
        Uint::<256, 4>::from_be_bytes(fee_growth_global.fee_growth_global_0_x128.to_be_bytes()),
        Uint::<256, 4>::from_be_bytes(fee_growth_global.fee_growth_global_1_x128.to_be_bytes()),
    );

    let new_position = match position {
        Some(mut position) => {
            let (token_owned_0, token_owned_1) = get_tokens_owed(
                Uint::<256, 4>::from_be_bytes(position.fee_growth_inside_0_last_x128.to_be_bytes()),
                Uint::<256, 4>::from_be_bytes(position.fee_growth_inside_1_last_x128.to_be_bytes()),
                position.liquidity,
        ...
        None => Position {
            ...
            fee_growth_inside_0_last_x128: Uint256::from_be_bytes(
                fee_growth_inside_0_x128.to_be_bytes(),
            ),
            fee_growth_inside_1_last_x128: Uint256::from_be_bytes(
    ...
    if liquidity_delta != 0 {
        let (amount_0, amount_1) = if current_state.tick < tick_lower {
            ...
            (
                get_amount_0_delta(
                    Uint::<256, 4>::from_be_bytes(current_state.sqrt_price_x96.to_be_bytes()),
                    get_sqrt_ratio_at_tick(tick_upper).unwrap(),
                    u128::try_from(liquidity_delta.abs()).unwrap(),
                    false,
                )
...
fn execute_mint(
    ...
    messages.push(WasmMsg::Execute {
        ...
        msg: to_json_binary(&Cw721BaseExecuteMsg::<NftExtentions>::Mint {
...
//         if !(sqrt_price_limit_x96 < current_state.sqrt_price_x96
//             && sqrt_price_limit_x96 > min_sqrt_ratio_in_uint256)
//         {
...
//         if !(sqrt_price_limit_x96 > current_state.sqrt_price_x96
//             && sqrt_price_limit_x96 < max_sqrt_ratio_in_uint256)
//         {
//             return Err(ContractError::InvalidSqrtPrice {});
//         }
//     }

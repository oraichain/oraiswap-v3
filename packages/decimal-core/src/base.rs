...
pub fn generate_base(characteristics: DecimalCharacteristics) -> proc_macro::TokenStream {
    ...
    proc_macro::TokenStream::from(quote!(
        impl Decimal for #struct_name {
            ...
            fn checked_one<T: TryFrom<u128>>() -> core::result::Result<T, String> where
                T::Error: core::fmt::Display,
            {
                T::try_from(#denominator).map_err(|err| format!("checked_one: can not get one to type {} : {}", core::any::type_name::<T>(), &err))
            }
